...
const glowVariants = {
  initial: { scale: 1, y: 0, filter: "drop-shadow(0 0 0px rgba(0,0,0,0))" }, // Fix 2: Removed space
  hover: {
    scale: 1.2,
    y: -3,
...
const Footer = () => {
  ...
  {socials.map(({ Icon, label, href }) => ( 
    <motion.a 
      ...
      variants={glowVariants} // Fixed spelling from "varients"
      initial="initial"
      whileHover="hover"
      whileTap="tap"
      className="text-gray-300 transition-colors duration-200 inline-flex items-center"
    >
      <Icon /> 
    </motion.a>
  ))}
...
<p  className='text-xs text-gray-400'>
  &copy; {new Date().getFullYear()}Rohit Gupta. All rights reserved.
</p>
    </motion.div>
   </footer>
  )
